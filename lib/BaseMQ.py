...
class BaseMQ(object):
    ...
    def calibrate(self, ro=-1):
        if ro == -1:
            ...
            for i in range(0,MQ_SAMPLE_TIMES + 1):        
                ...
                ro += self.__calculateResistance__(self.pinData()) #(self.pinData.read())
    ...
    def __calculateResistance__(self, rawAdc):
        #print(rawAdc)
        vrl = rawAdc*(self._baseVoltage / 4095) #1023
    ...
    def __readRs__(self):
        if self.measuringStrategy == STRATEGY_ACCURATE :            
                rs = 0
                for i in range(0, MQ_SAMPLE_TIMES + 1): 
                    rs += self.__calculateResistance__(self.pinData()) #pinData.read())
                    utime.sleep_ms(MQ_SAMPLE_INTERVAL)
    ...
    def readScaled(self, a, b):   
        #print(134/self.readRatio()*.25-35)     
